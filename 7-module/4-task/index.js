...
export default class StepSlider {
  ...
  render() {
    ...
    sliderThumb.ondragstart = () => false; // выключить встроенный браузерный Drag-and-Drop
  ...
  onPointerDown = (event) => {
    ...
    thumb.style.zIndex = 300;
    document.addEventListener('pointermove', this.onPointerMove);        
  }
  // описание функции перемещений
  onPointerMove = (event) => {
    ...
    thumb.style.left = `${leftPercents}%`;  // подвинуть ползунок
    progress.style.width = `${leftPercents}%`; // покрасить прогресс
    
    this.elem.classList.add('slider_dragging'); // после начала перетаскивания ползунка, корневому элементу слайдера нужно добавить класс slider_dragging
  ...
  // действия при отжатии кнопки
  onPointerUp = (event) => {
    
    document.removeEventListener('pointermove', this.onPointerMove);  // убрать обработчик
    
    this.elem.classList.remove('slider_dragging'); // убрать класс
    
    let change = new CustomEvent('slider-change', { // генерация customevent
      detail: this.value, 
      bubbles: true 
    });
